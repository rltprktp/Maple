...
public class Simulator extends JFrame {
	...
	public void imgch(int i, JButton btn) {
		icon = new ImageIcon(i+".jpg");
		img = icon.getImage();
	...
	private void showFrame() {
		...
		JPanel pnlInNorth = new JPanel();
		pnlInNorth.add(lblCurrentLevel, BorderLayout.WEST);
		pnlInNorth.add(lblCurrentNum, BorderLayout.EAST);
		pnlInNorth.setBackground(Color.WHITE);

		JPanel pnlInSouth = new JPanel();
		pnlInSouth.add(lblexp, BorderLayout.WEST);
		pnlInSouth.add(lblexpNum, BorderLayout.CENTER);
		pnlInSouth.add(lblexpPer, BorderLayout.EAST);
		pnlInSouth.setBackground(Color.WHITE);

		JPanel pnlNorth = new JPanel();
		pnlNorth.add(pnlInNorth);
		pnlNorth.add(pnlInSouth);
		...
		btn209.addActionListener((e) -> {
			...
			if (level < 210) {
				...
				exp += exp209;				if (exp >= Exp.get(level)) {
					exp -= Exp.get(level);
					level++;
					changeIntString(level);
					lblCurrentNum.setText(i_str);
